...
class Translator(object):
    ...
    def buildData(self, srcBatch, goldBatch, guideBatch):
        ...
        guideData = [self.guide_dict.convertToIdx(b, S2S.Constants.UNK_WORD) for b in guideBatch]
        tgtData = None
        if goldBatch:
        ...
        return S2S.Dataset(srcData, tgtData, None, None, self.opt.batch_size, self.opt.cuda, guideData)
    ...
    def buildTargetTokens(self, pred, src, isCopy, copyPosition, attn):
        ...
        if self.opt.replace_unk:
            for i in range(len(tokens)):
                if tokens[i] == S2S.OtherConstants.UNK_WORD:
    ...
    def translateBatch(self, srcBatch, tgtBatch, guideData):
        ...
        encStates, context, backward_hids, wordEmb = self.model.encoder(srcBatch, guideData)
        ...
        beam = [S2S.Beam(beamSize, srcLength[0][k], self.opt.cuda) for k in range(batchSize)]
