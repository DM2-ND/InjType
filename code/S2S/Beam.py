...
class Beam(object):
    ...
    def advance(self, wordLk, copyLk, attnOut):
        ...
        if all(self.nextYs[-1].eq(S2S.Constants.EOS)):
            self.done = True
        '''
        if all(self.nextYs[-1].eq(S2S.Constants.SS)):
            self.ends += 1

        if all(self.nextYs[-1].eq(S2S.Constants.SS)) and self.ends == self.ents+1:
            self.done = True
        '''
        if all(self.nextYs[-1].eq(S2S.Constants.EOS)) and self.ends != self.ents+1:
