...
class Beam(object):
    def __init__(self, size, srcLength, cuda=False):
        ...
        self.nextYs = [self.tt.LongTensor(size).fill_(seq2seq.Constants.PAD)]
        self.nextYs[0][0] = seq2seq.Constants.BOS
        self.nextYs_true = [self.tt.LongTensor(size).fill_(seq2seq.Constants.PAD)]
        self.nextYs_true[0][0] = seq2seq.Constants.BOS
    ...
    def advance(self, wordLk, copyLk, attnOut):
        ...
        if len(self.prevKs) > 0:
            finish_index = self.nextYs[-1].eq(seq2seq.Constants.EOS)
            if any(finish_index):
                ...
                for i in range(self.size):
                    if self.nextYs[-1][i] == seq2seq.Constants.EOS:
                        # wordLk[i][seq2seq.Constants.EOS] = 0
                        allScores[i][seq2seq.Constants.EOS] = 0
            # set up the current step length
            cur_length = self.all_length[-1]
            for i in range(self.size):
                cur_length[i] += 0 if self.nextYs[-1][i] == seq2seq.Constants.EOS else 1
        ...
        final_predict = predict * (1 - isCopy) + isCopy * seq2seq.Constants.UNK
        ...
        '''
        if all(self.nextYs[-1].eq(seq2seq.Constants.EOS)):
            self.done = True
        '''
        if all(self.nextYs[-1].eq(seq2seq.Constants.SS)):
            self.ends += 1

        if all(self.nextYs[-1].eq(seq2seq.Constants.SS)) and self.ends == self.ents+1:
            self.done = True
        '''
        if all(self.nextYs[-1].eq(seq2seq.Constants.EOS)) and self.ends != self.ents+1:
            print('{} {}'.format(self.ends,self.ents+1))
